...
def relevant_state(db, source_id, destination_concept_id=None, target_id=None):
    source = rows(db, "SELECT alternative_id,concept_id,retired_at IS NULL AS active FROM alternative WHERE alternative_id=?", (source_id,))
    ...
    return {
        "source": source,
        "concepts": rows(db, f"SELECT concept_id FROM concept WHERE concept_id IN ({marks}) ORDER BY concept_id", ids),
        "alternatives": alternatives,
        "assignments": rows(db, f"SELECT assignment_id,occurrence_id,alternative_id FROM assignment WHERE alternative_id IN ({amarks}) AND is_current=1 ORDER BY assignment_id", aids),
        "evidence": rows(db, f"""SELECT o.occurrence_id,o.occurrence_year,
            s.source_id,s.start_year,s.end_year,s.end_year_status,
        ...
        "relations": rows(db, f"SELECT alternative_relation_id,alternative_low_id,alternative_high_id,phonological_parameter FROM alternative_relation WHERE is_current=1 AND (alternative_low_id IN ({amarks}) OR alternative_high_id IN ({amarks})) ORDER BY alternative_relation_id", aids + aids),
        "morphology": rows(db, f"SELECT alternative_morphology_id,alternative_id,component_count,component_count_not_applicable,free_permutation,note FROM alternative_morphology WHERE alternative_id IN ({mmarks}) AND is_current=1 ORDER BY alternative_morphology_id", mids),
