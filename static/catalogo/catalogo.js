...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  const filtrar=()=>{const consulta=normalizar(buscador?.value);const soloVideo=Boolean(filtroVideo?.checked);let visibles=0;conceptos.forEach(elemento=>{const visible=(!consulta||normalizar(elemento.dataset.conceptoBusqueda).includes(consulta))&&(!soloVideo||elemento.dataset.hasVideo==="true");elemento.classList.toggle("oculto",!visible);if(visible)visibles+=1});document.querySelectorAll(".selector-alternativas [data-has-video]").forEach(elemento=>elemento.classList.toggle("oculto",soloVideo&&elemento.dataset.hasVideo!=="true"));document.querySelectorAll("[data-variation-group]").forEach(grupo=>grupo.classList.toggle("oculto",soloVideo&&![...grupo.querySelectorAll("[data-has-video]")].some(item=>item.dataset.hasVideo==="true")));if(contador)contador.textContent=`${visibles} concepto(s)`};
  buscador?.addEventListener("input",filtrar);filtroVideo?.addEventListener("change",filtrar);filtrar();
  ...
  document.querySelectorAll(".bloque-red-variacion").forEach(detalle=>detalle.addEventListener("toggle",()=>{const texto=detalle.querySelector(".texto-plegado");if(texto)texto.textContent=detalle.open?"Ocultar":"Mostrar"}));
});
