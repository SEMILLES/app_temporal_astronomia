...
class ConflictHookTests(unittest.TestCase):
    ...
    def test_assignment_hook_is_atomic_with_canonical_rollback(self):
        self.db.execute("BEGIN IMMEDIATE");create_or_replace_assignment(self.db,1,1)
        self.assertEqual(1,self.db.execute("SELECT count(*) FROM conflict").fetchone()[0]);self.db.rollback()
