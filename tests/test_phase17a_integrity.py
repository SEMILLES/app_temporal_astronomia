...
class WorkflowIntegrityTests(unittest.TestCase):
        ...
        for count in (2, 3):
            with self.subTest(count=count):
                data = dict(
                    ...
                    morphology_component_count=str(count),
                    free_permutation="NO",
                    record_components="yes",
                    component_row_id=[str(i) for i in range(count)],
                )

                for i in range(count):
                    data.update({
                        f"component_{i}_position": str(i + 1),
                ...
                sid = db.execute(
                    'SELECT max(submission_id) FROM submission'
                ).fetchone()[0]

                rows = db.execute(
                    'SELECT * FROM alternative_submission_component '
                ...
                self.assertEqual(
                    [r['component_alternative_id'] for r in rows],
                    [1] + [None] * (count - 1),
                )
                ...
                version = db.execute(
                    ...
                    'WHERE created_from_submission_id=?',
                    (sid,),
                ).fetchone()

                self.assertEqual(
                    db.execute(
                        'SELECT count(*) FROM alternative_component '
                        'WHERE alternative_morphology_id=?',
                        (version['alternative_morphology_id'],),
                    ).fetchone()[0],
                    count,
                )
                ...
                components = [
                    dict(
                        position=r['position'],
                        component_alternative_id=r['component_alternative_id'],
                        note=r['note'],
                    )
                    for r in rows
                ]

                new_id, changed = create_or_replace_alternative_morphology(
                    db,
    ...
    def test_malformed_components_never_truncate(self):
        ...
        cases = [
            dict(
                ...
                component_type=['unapproved'],
                component_alternative_id=['', ''],
                component_note=['a', 'b'],
            ),
            dict(
                component_row_id=['0'],
                component_0_position='1',
                component_0_type='unapproved',
                component_0_note='a',
            ),
            dict(
                component_row_id=['0', '0'],
                component_0_position='1',
                component_0_type='unapproved',
                component_0_note='a',
                component_0_alternative_id='',
            ),
            dict(
    ...
    def test_browser_component_rows_have_independent_types(self):
        ...
        with sync_playwright() as playwright:
            ...
            page.locator(
                '[name=morphology_component_count]'
            ).select_option('3')

            page.locator(
                '[name=record_components][value=yes]'
            ).check()
            ...
            for i in range(3):
                ...
                page.locator(
                    f'[name=component_{i}_type]'
                    f'[value={"existing" if i == 0 else "unapproved"}]'
                ).check()

                if i == 0:
                    page.locator(
                        '[name=component_0_alternative_id]'
                    ).select_option('1')

                page.locator(
                    f'[name=component_{i}_note]'
                ).fill(f'Browser row {i}')

            self.assertEqual(
                page.locator('[data-component-type]:checked').count(),
                3,
            )

            values = page.locator('#components').evaluate(
                "node => Array.from("
                "new FormData(node.closest('form')).entries())"
            )
    ...
    def test_concept_permissions_and_atomic_history(self):
        ...
        for path, method in [
            ('/conceptos/nuevo', 'post'),
            ('/conceptos/1/editar', 'get'),
            ('/conceptos/1/actualizar', 'post'),
        ]:
            self.assertEqual(
                getattr(
                    self.client,
        ...
        for prefix, role in [
            ...
            self.assertEqual(
                self.client.post(
                    f'/{prefix}/conceptos/nuevo',
                    data={'preferred_label': prefix},
                ).status_code,
                302,
            )
            ...
            self.assertEqual(
                self.client.post(
                    f'/{prefix}/conceptos/1/actualizar',
                    data={
                        'preferred_label': prefix + ' renamed'
                    },
                ).status_code,
                302,
            )
            ...
            event = db.execute(
                ...
                "WHERE event_type='concept_renamed' "
                "ORDER BY activity_event_id DESC"
            ).fetchone()
            ...
            self.assertEqual(
                json.loads(event['comment'])['new_label'],
                prefix.upper() + '-RENAMED',
            )

            self.assertEqual(
                db.execute(
                    'SELECT concept_id FROM alternative '
...
class RealCopyBackfillTests(unittest.TestCase):
    def test_baseline_copy_and_idempotence(self):
        baseline = (
            ...
            / 'import_inputs/astronomia/'
              'lesico_astronomia_working_baseline_post_fase16b_2026-09-05.db'
        )
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            expected = [
                tuple(r)
                for r in db.execute(
                    'SELECT occurrence_id,concept_id '
                    'FROM assignment '
                    'JOIN alternative USING(alternative_id) '
            ...
            self.assertEqual(
                normalize(db, apply=True)['created'],
                253,
            )
            ...
            create_alternative_submission(
                ...
                expected[0][0],
                'UNSURE',
                analysis_note='Phase17A reanalysis smoke',
            )
