...
class AnalysisFlowTests(unittest.TestCase):
    ...
    def test_browser_single_form_relations_validation_and_unsure(self):
        ...
        with sync_playwright() as pw:
            ...
            page.locator('#immediate-confirm').click()
            duplicate_message = page.locator('form > p[role=alert]').inner_text()
            self.assertIn('No se puede registrar más de una relación con', duplicate_message)
            self.assertIn('C-1', duplicate_message)
            self.assertIn('Cada alternativa destino admite una sola relación fonológica y un único parámetro diferenciador', duplicate_message)
